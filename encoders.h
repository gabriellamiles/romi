// These #defines act like a find-and-replace in your code, and make your code more readable.
...
// This ISR handles just Encoder 1 ISR to read the Encoder1 Channel A and B pins
// and then look up based on  transition what kind of rotation must have occured.
ISR( INT6_vect ) {
  ...
  // Some clever electronics combines the signals and this XOR restores the
  // true value.
  newE1_A ^= newE1_B;
...
void drive_distance(int wheel_speed_left,  int wheel_speed_right, float target_ref_l, float target_ref_r) {
  
  if (target_ref_l < 0 && target_ref_r < 0) {
    digitalWrite(L_DIR_PIN, LOW);
    digitalWrite(R_DIR_PIN, LOW);
    l_speed = abs(wheel_speed_left);    
    r_speed = abs(wheel_speed_right);
    analogWrite(L_PWM_PIN, l_speed);
    analogWrite(R_PWM_PIN, r_speed);
    
  } else if (target_ref_l >= 0 && target_ref_r >=0) {
    
    //dist_tar_abs = abs(distance_target);
    digitalWrite(L_DIR_PIN, HIGH);
    digitalWrite(R_DIR_PIN, HIGH);
    l_speed = wheel_speed_left;
    r_speed = wheel_speed_right;
    analogWrite(L_PWM_PIN, l_speed);
    analogWrite(R_PWM_PIN, r_speed);
  ...
  } else if (target_ref_r >= 0 && target_ref_l < 0) {
